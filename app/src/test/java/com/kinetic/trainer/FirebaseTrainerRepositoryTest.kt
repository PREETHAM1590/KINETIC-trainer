...
class FirebaseTrainerRepositoryTest {
    ...
    @Test
    fun test_get_workout_templates_returns_fake_when_gym_id_empty() = runTest {
        every { mockSession.gymId } returns ""

        val templates = repository.getWorkoutTemplates()

        assertEquals(FakeTrainerData.workoutTemplates, templates)
    }

    @Test
    fun test_observe_client_roster_emits_fake_data_when_gym_id_empty() = runTest {
        every { mockSession.gymId } returns ""
        every { mockSession.trainerId } returns ""

        repository.observeClientRoster("trainer1").test {
            assertEquals(FakeTrainerData.clients, awaitItem())
            cancelAndIgnoreRemainingEvents()
        }
    }

    @Test
    fun test_observe_activity_feed_emits_fake_data_when_gym_id_empty() = runTest {
        every { mockSession.gymId } returns ""

        repository.observeActivityFeed("trainer1").test {
            assertEquals(FakeTrainerData.activityFeed, awaitItem())
            cancelAndIgnoreRemainingEvents()
        }
    }

    @Test
    fun test_observe_client_detail_emits_fake_data_when_gym_id_empty() = runTest {
        every { mockSession.gymId } returns ""

        repository.observeClientDetail("c1").test {
            assertEquals(FakeTrainerData.clientDetails["c1"], awaitItem())
            cancelAndIgnoreRemainingEvents()
        }
    }

    @Test
    fun test_observe_chat_messages_emits_fake_data_when_not_ready() = runTest {
        every { mockSession.gymId } returns ""
        every { mockSession.trainerId } returns ""

        repository.observeChatMessages("c1").test {
            val expected = FakeTrainerData.chatMessages["c1"] ?: emptyList()
            assertEquals(expected, awaitItem())
            cancelAndIgnoreRemainingEvents()
        }
    }

    // ─── observeClientRoster — Firestore snapshot behaviour ──────────────────
    ...
    @Test
    fun test_get_workout_templates_returns_fake_on_firestore_exception() = runTest {
        every { mockSession.gymId } returns "gym1"
        ...
        assertEquals(
            "Exception during getWorkoutTemplates must fall back to fake templates",
            FakeTrainerData.workoutTemplates, templates
        )
    }
}
